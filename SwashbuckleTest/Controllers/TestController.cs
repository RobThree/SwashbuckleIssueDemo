...
namespace SwashbuckleTest.Controllers
{
    ...
    public class TestController : ControllerBase
    {
        ...
        public string GetQuerystring1(string id, string name)
            => BuildResult(id, name);

        // Shows "IsNew" as argument name
        [HttpGet("testgetquerystring2")]
        public string GetQuerystring2([FromQuery] string id, string name)
            => BuildResult(id, name);

        // Same as above, now with name specified but is ignored
        [HttpGet("testgetquerystring3")]
        public string GetQuerystring3([FromQuery(Name = "id")] string id, string name)
            => BuildResult(id, name);
